...
- Changed: JSON handling hardened with `JSON_THROW_ON_ERROR` (+ `JSON_UNESCAPED_SLASHES` on encode). `OnPayAPI::post()` throws `ApiException` on an unencodable body (was: sent a null body); a corrupt stored token throws `TokenException` from both `get()` and `post()` (1.x let the OAuth client's own exception escape from `post()`).
- Fixed: `TransactionHistory` and `SubscriptionHistory` now read `uuid` (always present and non-null on the wire) as a required field, and `DetailedSubscription` reads `card_mask` (present but nullable) — all previously dropped; implicitly-nullable constructor params are explicitly nullable (clears PHP 8.4+ deprecations); stale Guzzle `@throws` docblocks are removed; `InvalidCartException::$errors` is now a typed `array`.
...
- Fixed: `PaymentWindow::validatePayment()` no longer throws a `TypeError` when the callback query it is given contains a numeric key (`?0=x`, which PHP turns into an int key) or an array-valued `onpay_hmac_sha1` (`?onpay_hmac_sha1[]=x`). Non-string keys are skipped and a non-string HMAC returns `false`, so the raw `$_GET` can be passed as-is.
- Added: OAuth `state` (CSRF) verification and PKCE. Implement `OnPay\AuthStateStorageInterface` and pass it as the third `OnPayAPI` constructor argument (immediately after `$options`) to persist the `state` and PKCE `code_verifier` across the redirect; `finishAuthorize()` gains an optional second argument for the returned `state` and throws `TokenException` on a mismatch before any token exchange. See UPGRADE.md.
...
- Added: consistently named `PaymentWindow` accessors — `isSurchargeEnabled(): ?bool` (the old `isSurcharge_enabled()` was the SDK's only snake/camel hybrid) and a typed test-mode pair, `setTestModeEnabled(bool): void` / `isTestModeEnabled(): bool`, replacing the untyped `setTestMode()`/`getTestMode()` (whose return was `int|bool|string|null`). See UPGRADE.md.
- Deprecated: `PaymentWindow::isSurcharge_enabled()`, `setTestMode()` and `getTestMode()` are `@deprecated` aliases of the above with unchanged behaviour; they are the 2.0 migration path and are not removed in this release. See UPGRADE.md.
- Removed: everything that already carried a `@deprecated` tag in 1.x. `PaymentWindow::setSecureEnabled()`/`hasSecureEnabled()` are gone — use `set3DSecure()`/`is3DSecure()`, which they only delegated to. So are `Transaction\CardholderData`'s eight split address-component properties, `$street`/`$number`/`$floor`/`$door` and their `$delivery*` counterparts; use `$address1`/`$address2` and `$deliveryAddress1`/`$deliveryAddress2`. See UPGRADE.md.
- BREAKING: internal-by-default. Everything that is not a deliberate part of the consumer-facing surface is now marked `@internal`, and every class in `src/` that is neither abstract nor extended by the SDK itself is `final` — 48 classes, including `PaymentWindow`, `PaymentWindow\Cart`, `PaymentWindow\CartItem`, `PaymentWindow\PaymentInfo`, the four API service classes, every leaf response value object, and all transport/OAuth/logging plumbing. `Transaction\SimpleTransaction` and `Subscription\SimpleSubscription` stay open (their `Detailed*` variants extend them), as do the abstract `Exception\OnPayException` and `PaymentMethodAbstract` bases and the interfaces you implement (`TokenStorageInterface`, `AuthStateStorageInterface`). `Util\PaymentMethods\Methods\PaymentMethodInterface` is now `@internal` like the method classes that implement it; it remains the return type of `Currency::getPaymentMethods()` and `PaymentMethods::getAllPaymentMethods()`. Psalm's `ClassMustBeFinal` check enforces this in CI. See UPGRADE.md.
- BREAKING: member visibility now matches the contract. `PaymentWindow\PaymentInfo`'s 42 field properties plus `$availableFields` and `validateField()`, the `Http\Request`/`Http\Response` backing properties and `StaticToken::$staticToken` are `private` instead of `protected`. Together with `final` this only affects code that subclassed these classes. See UPGRADE.md.
- Changed: `PaymentWindow\PaymentInfo`'s validation patterns are now grouped before they are anchored, and carry the `D` modifier. `'Y|N'` previously interpolated to `/^Y|N$/u`, which PCRE reads as `(^Y)|(N$)`, so the five `Y`/`N` fields accepted any value starting with `Y` or ending with `N` (`'Yes'`, `'ON'`); without `D`, every field accepted a trailing newline (`"208\n"`). Both are now rejected with `InvalidFormatException`. Exactly `Y` or `N` is what the API accepts for the equivalent fields, so this brings the setters in line with the documented contract.
- Fixed: `PaymentWindow\PaymentInfo::setPhoneHome()`, `setPhoneMobile()` and `setPhoneWork()` no longer leave the object partially updated when they reject a value. Each validated and stored the country code before validating the number, so `setPhoneHome('45', 'notanumber')` threw `InvalidFormatException` with `phone_home_cc` already set. Both values are now validated before either is stored. The six private per-component setters they delegated to are gone, folded into the pair methods.
- BREAKING: `Util\Currencies::isValidISO4217()` now takes an `int` instead of `int|string`. It compared the stored codes with `===`, so a numeric string could never match and `isValidISO4217('208')` returned `false` while `isValidISO4217(208)` returned `'DKK'` — the signature promised something it could not do. Numeric codes are ints on the wire (`currency_code`), so the signature now says so: a strict-mode caller passing `'208'` gets a `\TypeError`, a weak-mode caller has it coerced and now succeeds. `Util\Currency` dispatches on the argument type and is unchanged, including `new Currency('208')` still throwing `ApiException`. See UPGRADE.md.
- Changed: `Util\Currencies::isValidAlpha3()` is now case-insensitive and returns the canonical uppercase code, so `isValidAlpha3('dkk')` gives `'DKK'` where it previously gave `false`, and `new Currency('dkk')->getAlpha3()` is `'DKK'`. Matches the API, which accepts a lowercase `currency` on input but always returns it uppercase, and `PaymentMethods::getCurrenciesByMethod()`, which already folded case. Widens what is accepted, so existing calls are unaffected.
- Fixed: a 2xx response whose `Content-Type` is present and not JSON now throws an `ApiException` naming the cause, for example `Expected a JSON body, got text/html with HTTP 200`. 1.x decoded such a body to nothing and failed with a `TypeError` while building the result. A response with no `Content-Type` is still decoded, and `application/json; charset=utf-8` is unaffected.
- Changed: the transaction response value objects now document the response shapes that are easy to model wrongly, with tests pinning each: `DetailedTransaction::$fee` is absent unless the endpoint reports a surcharge (null means "not reported", `0` means no surcharge); `$cardCountry`/`$ipCountry` are zero-padded ISO 3166 numeric *strings* (`"004"`) while `SimpleTransaction::$currencyCode` is an *int* and `CardholderData::$country` is an int for the same kind of code; `$hasCardholderData` can be true with `$cardholderData` null; `CardholderData::$extraFields` is an array whether the value arrives as `[]` or `{}`; and `$subscriptionNumber`/`$subscriptionUuid` are null unless the transaction is merchant-initiated. No runtime behaviour changes. The cardholder fixture's `country` values are corrected to the ints the API sends.
