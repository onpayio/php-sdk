...
- BREAKING: internal-by-default. Everything that is not a deliberate part of the consumer-facing surface is now marked `@internal`, and every class in `src/` that is neither abstract nor extended by the SDK itself is `final` — 48 classes, including `PaymentWindow`, `PaymentWindow\Cart`, `PaymentWindow\CartItem`, `PaymentWindow\PaymentInfo`, the four API service classes, every leaf response value object, and all transport/OAuth/logging plumbing. `Transaction\SimpleTransaction` and `Subscription\SimpleSubscription` stay open (their `Detailed*` variants extend them), as do the abstract `Exception\OnPayException` and `PaymentMethodAbstract` bases and the interfaces you implement (`TokenStorageInterface`, `AuthStateStorageInterface`). `Util\PaymentMethods\Methods\PaymentMethodInterface` is now `@internal` like the method classes that implement it; it remains the return type of `Currency::getPaymentMethods()` and `PaymentMethods::getAllPaymentMethods()`. Psalm's `ClassMustBeFinal` check is no longer suppressed, so CI enforces this going forward. See UPGRADE.md.
- BREAKING: member visibility now matches the contract. `PaymentWindow\PaymentInfo`'s 42 field properties plus `$availableFields` and `validateField()`, the `Http\Request`/`Http\Response` backing properties, `StaticToken::$staticToken` and `OAuth\OnPayProvider`'s `$urlAuthorize`/`$urlAccessToken`/`$pkceEnabled` are `private` instead of `protected`; the unused public `Http\LoggingHttpClient::getInnerClient()` is removed. Together with `final` this only affects code that subclassed these classes. See UPGRADE.md.
- BREAKING: `PaymentWindow\PaymentInfo`'s validation patterns are now grouped before they are anchored, and carry the `D` modifier. `'Y|N'` previously interpolated to `/^Y|N$/u`, which PCRE reads as `(^Y)|(N$)`, so the five `Y`/`N` fields accepted any value starting with `Y` or ending with `N` (`'Yes'`, `'ON'`); without `D`, every field accepted a trailing newline (`"208\n"`). Both are now rejected with `InvalidFormatException`. Exactly `Y` or `N` is what the API accepts for the equivalent fields, so this brings the setters in line with the documented contract. See UPGRADE.md.
...
- Fixed: a 2xx response whose `Content-Type` is present and not JSON now throws an `ApiException` naming the cause, instead of the misleading `Failed to decode JSON body-response: Syntax error`. The API can answer a lookup for a resource that does not exist with HTTP 200 and an HTML error page rather than a 404. A response with no `Content-Type` is still decoded, and `application/json; charset=utf-8` is unaffected. See UPGRADE.md.
