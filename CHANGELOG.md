...
- Removed: everything that already carried a `@deprecated` tag in 1.x. `PaymentWindow::setSecureEnabled()`/`hasSecureEnabled()` are gone — use `set3DSecure()`/`is3DSecure()`, which they only delegated to. So are `Transaction\CardholderData`'s eight split address-component properties, `$street`/`$number`/`$floor`/`$door` and their `$delivery*` counterparts; use `$address1`/`$address2` and `$deliveryAddress1`/`$deliveryAddress2`. This one is a data-access removal, not a cleanup: the API still sends the underlying `street`/`number`/`floor`/`door` keys and the SDK now deliberately ignores them. See UPGRADE.md.
