...
- Added: consistently named `PaymentWindow` accessors — `isSurchargeEnabled(): ?bool` (the old `isSurcharge_enabled()` was the SDK's only snake/camel hybrid) and a typed test-mode pair, `setTestModeEnabled(bool): void` / `isTestModeEnabled(): bool`, replacing the untyped `setTestMode()`/`getTestMode()` (whose return was `int|bool|string|null`). `setTestModeEnabled(true)` is HMAC-identical to `setTestMode(true)`; `setTestModeEnabled(false)` omits `onpay_testmode` from the form fields instead of sending `0`, mirroring `set3DSecure(false)`. See UPGRADE.md.
- Deprecated: `PaymentWindow::isSurcharge_enabled()`, `setTestMode()` and `getTestMode()` are `@deprecated` aliases of the above with unchanged behaviour, and the long-standing `setSecureEnabled()`/`hasSecureEnabled()` deprecation tags now name their replacements (`set3DSecure()`/`is3DSecure()`). Nothing is removed. See UPGRADE.md.
