...
- BREAKING: every `src/` file now declares `strict_types=1` and the SDK's interfaces (`TokenStorageInterface` (both), `OAuth\Client\SessionInterface`, `OAuth\Client\Http\HttpClientInterface`) carry native types; classes that implement them must declare compatible types. Public methods gained native parameter types: in strict mode a mismatched scalar raises `TypeError`, and in any mode passing `null` to a now non-nullable parameter (e.g. the `PaymentWindow` setters) raises `TypeError`. See UPGRADE.md.
